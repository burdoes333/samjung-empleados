    ...
    <div style={{minHeight:"100vh",background:"#1a3a6b",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{background:"#fff",borderRadius:16,padding:24,width:"100%",maxWidth:340,boxShadow:"0 8px 32px rgba(0,0,0,0.2)"}}>
        ...
        <input
          ...
          placeholder="Ej: Angel Ahumada"
          style={{width:"100%",padding:"10px 12px",borderRadius:8,border:"0.5px solid #ddd",fontSize:14,marginBottom:16,background:"#f5f7fa"}}
        />

        <label style={{fontSize:11,color:"#888",display:"block",marginBottom:8}}>PIN</label>
        <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:16}}>
          {[0,1,2,3,4,5].map(i=>(
            <div key={i} style={{width:36,height:36,borderRadius:"50%",border:`2px solid ${pin.length>i?"#1a3a6b":"#ddd"}`,background:pin.length>i?"#1a3a6b":"#f5f7fa",transition:"all 0.15s"}}/>
          ))}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:16}}>
          {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((d,i)=>(
            <button key={i} onClick={()=>d==="⌫"?handleDelete():d!==""?handlePinClick(String(d)):null}
              style={{padding:"14px",borderRadius:10,border:"0.5px solid #eee",background:d==="⌫"?"#fff3f3":d===""?"transparent":"#fff",fontSize:d==="⌫"?18:18,fontWeight:500,cursor:d===""?"default":"pointer",color:d==="⌫"?"#a32d2d":"#222",boxShadow:d!==""&&d!=="⌫"?"0 1px 4px rgba(0,0,0,0.06)":"none"}}>
              {d}
            </button>
          ))}
        </div>
